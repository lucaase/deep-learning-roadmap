...
df = pd.read_csv('FreeCodeCamp\FreeCodeCamp Data Analysis with Python\projects\\fcc-forum-pageviews.csv', parse_dates=['date'], index_col='date')

# Clean data
hi_treshold = df['value'].quantile(0.975)
lo_theshold = df['value'].quantile(0.025)
outliers = (df['value'] > hi_treshold) | (df['value'] < lo_theshold)
